...
def look_scheduling_info():
    os.system("cls")
    print(Fore.MAGENTA + """
          \t\t\t\t\t\t\t    ────────────୨ৎ─────────── 
          \t\t\t\t\t\t\t      𝐋𝐎𝐎𝐊 𝐒𝐂𝐇𝐄𝐃𝐔𝐋𝐈𝐍𝐆 𝐈𝐍𝐅𝐎    
          \t\t\t\t\t\t\t    ────────────୨ৎ───────────  
          """ + Fore.RESET)

    info_sections = [
        {"name": "What is LOOK?", "description": """
         - LOOK is a disk scheduling algorithm that moves the disk head toward the furthest request in the current direction.
         - Unlike SCAN, it does not go all the way to the end unless there is a request at the last track.
         - Once the head reaches the last request in that direction, it reverses direction.
         """},
        {"name": "Key Parameters", "description": """
         - Track Requests: List of track numbers that need to be accessed.
         - Head Position: Current position of the disk head.
         - Direction: Current movement direction (toward higher or lower track numbers).
         - Seek Sequence: Order in which tracks are serviced by the head.
         """},
        {"name": "Advantages", "description": """
         - Reduces unnecessary head movement compared to SCAN.
         - Efficient in handling requests distributed along the disk.
         - Simple to implement while providing good performance.
         """},
        {"name": "Implementation Notes", "description": """
         - Ensure all track requests are within valid range.
         - Decide the initial head direction before starting.
         - Useful for visualizing head movement to calculate total seek time.
         - Can be adapted for C-LOOK for circular head movement optimization.
         """}
    ]

    for section in info_sections:
        print(Fore.CYAN + f"{section['name']}:" + Fore.RESET, end="")
        print(f"{section['description']}")
    print()

    while True:
        try:
            ask_again = input(Fore.YELLOW + "𝗚𝗼 𝗕𝗮𝗰𝗸? [Y/N]: " + Fore.RESET)
